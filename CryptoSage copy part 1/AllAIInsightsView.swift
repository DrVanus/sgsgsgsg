        ...
        VStack(spacing: 4) {
            ...
            Rectangle()
                ...
                .frame(
                    width: 44,
                    height: maxScore > 0
                        ? (CGFloat(score) / CGFloat(maxScore)) * barHeight
                        : 0
                )
                .cornerRadius(4)
...
private extension AllAIInsightsView {
                ...
                ForEach(viewModel.contributors, id: \.name) { contributor in
                    HStack {
                        Text(contributor.name)
                            ...
                            .foregroundColor(.white)
                        Spacer()
                        Text(String(format: "%.0f%%", contributor.contributionPct * 100))
                            .font(.subheadline)
                            .foregroundColor(.white)
                    }
                    .padding(.horizontal, 12)
                ...
                VStack(alignment: .center) {
                    ...
                    HStack {
                        ...
                        HStack(alignment: .bottom, spacing: 4) {
                            ForEach(viewModel.tradeQualityData?.distributionBuckets.indices ?? 0..<0, id: \.self) { idx in
                                let count = viewModel.tradeQualityData?.distributionBuckets[idx] ?? 0
                                Rectangle()
                                    .fill(Color.yellow)
    ...
    var diversificationContent: some View {
        VStack(spacing: 14) {
            ...
            if let topAsset = viewModel.diversificationData?.assetWeights.max(by: { $0.weight < $1.weight }),
               topAsset.weight > 0.6 {
                Text("⚠️ Concentration Warning: \(Int(topAsset.weight * 100))% in \(topAsset.symbol)")
                    .font(.caption)
                    .foregroundColor(.red)
                    .padding(.horizontal, 12)
                    .padding(.vertical, 6)
    ...
    var momentumContent: some View {
        VStack(spacing: 12) {
            ...
            GeometryReader { geo in
                let strategies: [StrategyMomentum] = viewModel.momentumData?.strategies ?? []
                let maxScore = strategies.map(\.score).max() ?? 1.0
                let chartHeight = geo.size.height * 0.6
                ...
                ZStack {
                    ...
                    HStack(alignment: .bottom, spacing: 24) {
                        ForEach(strategies, id: \.symbol) { strategy in
                            MomentumBarView(
    ...
    var feeContent: some View {
        VStack(spacing: 14) {
            ForEach(viewModel.feeData?.fees ?? [], id: \.symbol) { feeItem in
                HStack {
                    Text(feeItem.symbol)
                        .font(.caption)
                        .foregroundColor(.white.opacity(0.8))
                    Spacer()
                    Text(String(format: "%.2f%%", feeItem.feePct * 100))
                        .font(.caption)
                        .foregroundColor(.white)
                }
                .padding(.horizontal, 12)
